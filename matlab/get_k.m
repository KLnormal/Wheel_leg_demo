...
[nRows, nCols, deg_plus1] = size(data);
deg = deg_plus1 - 1;
...
for i = 1:2
    for j = 1:6
        index = index + 1;
        coeffs = squeeze(data(i,j,:));  % 取出该元素的多项式系数（从最高次到常数项）

        % 构造一行 C 语言格式字符串
        str = sprintf('fit_K[%d] = ', index-1);
        for k = 1:length(coeffs)
            ...
            switch power
                case 4
                    term = sprintf('(%gf)*l4', a);
                case 3
                    term = sprintf('(%gf)*l3', a);
                case 2
                    term = sprintf('(%gf)*l2', a);
                case 1
                    term = sprintf('(%gf)*l1', a);
                case 0
                    term = sprintf('(%gf)', a);
            end
...
% userpath('clear');
% userpath('D:\Github\Wheel_leg_demo\matlab');
% 
% tic
% j=1;
% leg=0.05:0.01:0.15;
